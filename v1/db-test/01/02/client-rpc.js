...
const main = async () => {
    ...
    hswarm.on("connection", async (socket, details) => {
        ...
        socket.on("data", async (data) => {
            ...
            const result1 = await hrpcClient.request("put", Buffer.from(JSON.stringify({key: "foo", value: Math.random().toString()})));
            console.log("Result:", JSON.parse(result1));
            
            const result2 = await hrpcClient.request("get", Buffer.from(JSON.stringify({key: "foo"})));
